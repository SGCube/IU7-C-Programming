...
int read(matrix_t ma, FILE *f)
{
	...
	ma->a = calloc(ma->ro, sizeof(item_t*));
	if (!ma->a)
		return ERR_MEMORY;
	for (int i = 0; i < ma->ro; i++)
	{
		ma->a[i] = calloc(ma->co, sizeof(item_t));
		if (!ma->a[i])
			return ERR_MEMORY;
	}
